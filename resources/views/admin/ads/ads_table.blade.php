				...
				<div class="menu">
					...
					<div class="edit">
						<form action="{{url('ad').'/'.$ad->id.'/edit'}}" method="get">
							<button class="btn btn-outline-dark" onclick="confirmEdit()" {{Auth::user()->hasRole(['admin', 'super_admin']) ? '' : 'disabled'}} >Edit</button>
						</form>
					</div>
					{{-- @endhasanyrole
					...
					<div class="delete">
						<form action="{{url('ad') .'/'.$ad->id}}" method="post">
							@csrf
							{{ method_field('delete') }}
							<button class="btn btn-outline-danger" onclick="confirmDelete()" {{Auth::user()->hasRole(['admin', 'super_admin']) ? '' : 'disabled'}}>Delete</button>
						</form>
					</div>
					{{-- @endhasanyrole --}}
				</div>
